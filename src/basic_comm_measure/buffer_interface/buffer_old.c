...
#define CON_PACKING_COUNT_MAX 256
...
void requester_buffer_fin()
{
    ...
    cudaFree(comm_data0);
    cudaFree(comm_data1);
    cudaFree(packed_data_byte_D);
    cudaFree(src_data_offset_D);
    cudaFree(width_byte_D);
...
void setup_conditions(int *worker_init_info, int *requester_data_division)
{
    ...
    err = cudaMalloc((void **)&packed_data_byte_D, CON_PACKING_COUNT_MAX * sizeof(size_t));
    if (err != cudaSuccess)
    {
        printf("CUDA error:  cudaMalloc packed_data_byte_D: %s\n", cudaGetErrorString(err));
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    err = cudaMalloc((void **)&src_data_offset_D, CON_PACKING_COUNT_MAX * sizeof(size_t));
    if (err != cudaSuccess)
    {
        printf("CUDA error:  cudaMalloc src_data_offset_D: %s\n", cudaGetErrorString(err));
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    err = cudaMalloc((void **)&width_byte_D, CON_PACKING_COUNT_MAX * sizeof(size_t));
    if (err != cudaSuccess)
    {
        printf("CUDA error:  cudaMalloc width_byte_D: %s\n", cudaGetErrorString(err));
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
...
void requester_write_onestep_to_realbuffer(void *src_data, int buffer_index)
{
    ...
    for (int i = 0; i < con_packing_count; i++)
    {
        ...
        width_byte[i] = width_element[i] * unitsize;
        packed_data_element0[i] = height_element[i] * width_element[i];
        packed_data_byte[i] = packed_data_element0[i] * unitsize;
        src_data_offset[i] = requester_write_info_list[i].src_data_offset;
    ...
    cudaMemcpy(packed_data_byte_D, packed_data_byte, con_packing_count * sizeof(size_t), cudaMemcpyHostToDevice);
    cudaMemcpy(src_data_offset_D, src_data_offset, con_packing_count * sizeof(size_t), cudaMemcpyHostToDevice);
    cudaMemcpy(width_byte_D, width_byte, con_packing_count * sizeof(size_t), cudaMemcpyHostToDevice);
    call_con_pack_data(comm_data0, src_data, src_data_offset_D, packed_data_byte_D, packed_data_byte[0], width_byte_D, square_byte, con_packing_count, packing_stream);
    ...
    int loop_ctr_max = requester_write_times / con_packing_count;
    for (int loop_ctr = 1; loop_ctr < loop_ctr_max; loop_ctr++)
    {
        ...
        for (int i = 0; i < con_packing_count; i++)
        {
            ...
            packed_data_element0[i] = height_element[i] * width_element[i];
            packed_data_byte[i] = packed_data_element0[i] * unitsize;
            src_data_offset[i] = requester_write_info_list[loop_ctr * con_packing_count + i].src_data_offset;
            ...
            // printf("loop_ctr=%d, i=%d,rank=%d,height_element=%zu,width_element=%zu,width_byte=%zu,packed_data_element0=%zu,packed_data_byte=%zu,src_data_offset=%zu,target_data_offset0=%zu,target_rank0=%d\n", loop_ctr, i, submyrank, height_element[i], width_element[i], width_byte[i], packed_data_element0[i], packed_data_byte[i], src_data_offset[i], target_data_offset0[i], target_rank0[i]);
        }
        ...
        // if (submyrank == 0)
        // {
        //     printf("loop_ctr=%d,packing_time_ctr=%d, all_garea_write_time_ctr=%d\n", loop_ctr, packing_time_ctr, all_garea_write_time_ctr);
        // }
    }
    clock_gettime(CLOCK_MONOTONIC, &packing_start_time);
    cudaStreamSynchronize(packing_stream);
...
void requester_print_detail_write_time()
{
    ...
    buffer_reduce_max_gtime(write_polling_time_list, write_polling_max_ranktime, write_time_ctr, CTCA_subcomm);
    buffer_reduce_max_gtime(write_time_list, write_max_ranktime, write_time_ctr, CTCA_subcomm);

    buffer_reduce_max_gtime(packing_time_list, packing_max_ranktime, packing_time_ctr, CTCA_subcomm);
    buffer_reduce_max_gtime(all_garea_write_time_list, all_garea_write_max_ranktime, all_garea_write_time_ctr, CTCA_subcomm);
    if (submyrank == 0)
    {
        ...
        //  }
        // for (int i = 0; i < packing_time_ctr; i++)
        // {
        //     printf("packing_time_ctr[%d]: %f\n", i, packing_time_list[i]);
        // }
        // for (int i = 0; i < all_garea_write_time_ctr; i++)
        // {
        //     printf("all_garea_write_time_ctr[%d]: %f\n", i, all_garea_write_time_list[i]);
        // }
        ...
        buffer_calc_measure_time(write_polling_max_ranktime, write_time_ctr, &write_polling_ave_gtime, &write_polling_max_gtime, &write_polling_min_gtime);
        buffer_calc_measure_time(write_max_ranktime, write_time_ctr, &write_ave_gtime, &write_max_gtime, &write_min_gtime);
        buffer_calc_measure_time(packing_time_list, packing_time_ctr, &packing_ave_gtime, &packing_max_gtime, &packing_min_gtime);
        buffer_calc_measure_time(all_garea_write_time_list, all_garea_write_time_ctr, &all_garea_write_ave_gtime, &all_garea_write_max_gtime, &all_garea_write_min_gtime);
        ...
        printf("req packing_time: ave=%f, max=%f, min=%f sec\n", packing_ave_gtime, packing_max_gtime, packing_min_gtime);
        printf("req all_garea_write_time: ave=%f, max=%f, min=%f sec\n", all_garea_write_ave_gtime, all_garea_write_max_gtime, all_garea_write_min_gtime);
    }
...
void count_buffer_index()
{
    if (submyrank == 0)
    {
        ...
        printf("packing_time_ctr=%d\n", packing_time_ctr);
        printf("all_garea_write_time_ctr=%d\n", all_garea_write_time_ctr);
