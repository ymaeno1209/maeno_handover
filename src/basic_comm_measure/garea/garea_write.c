...
int sbuf_original[MYBUFSIZE];
int rbuf_original[MYBUFSIZE];
...
int main(int argc, char *argv[])
{
    ...
    case FLUSH_LOCAL:
        ...
        break;
    default:
        run_put_with_flush(rank, win_type); // この関数が呼ばれているはず
        break;
#endif
    }
    CTCAR_garea_delete();
...
void print_latency(int rank, int size)
{
    ...
    {
        fprintf(stdout, "%d, %f\n", size,
                (t_end - t_start) * 1.0e6 / options.loop);
        fflush(stdout);
    }
}
...
void run_put_with_flush(int rank, WINDOW type)
{
    ...
    {
        allocate_memory(rank, sbuf_original, rbuf_original, &sbuf, &rbuf, &rbuf, size, type, &win);
        CTCAR_garea_attach(gareaid, rbuf, size);
        ...
        if (rank == 0)
        {
            ...
            for (i = 0; i < options.skip + options.loop; i++)
            {
                ...
                CTCAR_garea_write_int(gareaid, target_rank, offset, size, sbuf);
                // MPI_CHECK(MPI_Put(sbuf, size, MPI_CHAR, 1, disp, size, MPI_CHAR, win));
        ...
        print_latency(rank, size);
        CTCAR_garea_detach(gareaid, rbuf);
        free_memory(sbuf, rbuf, win, rank);
    }
}
