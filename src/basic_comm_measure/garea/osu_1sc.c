...
void usage(int options_type, char const *name)
{
...
int process_options(int argc, char *argv[], WINDOW *win, SYNC *sync, int options_type)
{
    extern char *optarg;
    extern int optind;
    extern int opterr;
...
#if MPI_VERSION >= 3
        if ((options.rank0 == 'D' || options.rank1 == 'D') && *win == WIN_ALLOCATE)
        {
            *win = WIN_CREATE;
        }
#endif
...
int allocate_device_buffer(char **buffer, int size)
{
#ifdef _ENABLE_CUDA_
...
void allocate_memory(int rank, int *sbuf_orig, int *rbuf_orig, int **sbuf, int **rbuf,
                     int **win_base, int size, WINDOW type, MPI_Win *win)
{
    int page_size;
    ...
    if (mem_on_dev)
    {
        CHECK(allocate_device_buffer((void **)sbuf, size * sizeof(int)));
        set_device_memory(*sbuf, 0, size * sizeof(int));
        CHECK(allocate_device_buffer((void **)rbuf, size * sizeof(int)));
        set_device_memory(*rbuf, 1, size * sizeof(int));
    }
    else
    {
        *sbuf = (int *)align_buffer((void *)sbuf_orig, page_size);
        memset(*sbuf, 0, size * sizeof(int));
        *rbuf = (int *)align_buffer((void *)rbuf_orig, page_size);
        memset(*rbuf, 1, size * sizeof(int));
    }
...
void free_memory(void *sbuf, void *rbuf, MPI_Win win, int rank)
{
    MPI_Win_free(&win);
