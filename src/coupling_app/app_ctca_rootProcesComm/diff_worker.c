...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        for (int y = 0; y < num; y++)
        {
            ...
            // debug
            // double arrival_time = MPI_Wtime(); // 各プロセスがこの行に到達した時間を記録
            // struct
            // {
            //     double time;
            //     int rank;
            // } local_max, global_max;
            // local_max.time = arrival_time;
            // local_max.rank = new_rank;
            // MPI_Reduce(&local_max, &global_max, 1, MPI_DOUBLE_INT, MPI_MAXLOC, 0, CTCA_subcomm);
            // struct
            // {
            //     double time;
            //     int rank;
            // } local_min, global_min;
            // local_min.time = arrival_time;
            // local_min.rank = new_rank;
            // MPI_Reduce(&local_min, &global_min, 1, MPI_DOUBLE_INT, MPI_MINLOC, 0, CTCA_subcomm);
            // if (new_rank == 0)
            // {
            //     double max_min_diff = global_max.time - global_min.time; // 最も遅いプロセスと最も早いプロセスの時間差
            //     printf("a: step=%d, 最も遅く到達: プロセス %d , "
            //            "最も早く到達: プロセス %d , "
            //            "時間差: %f 秒\n",
            //            step, global_max.rank,
            //            global_min.rank,
            //            max_min_diff);
            //     prev_max_time = global_max.time; // 今回の最大時間を次のステップ用に保存
            // }
            if (new_rank == 0)
            {
                clock_gettime(CLOCK_MONOTONIC_RAW, &wrk_readarea_start_time);
                CTCAW_readarea_real4(areaid, fromrank, (size_t)intparams[1] * whole_surface_num, whole_surface_num, &y_axis_normal_plane_allprocs_H[num * num * y]);
                ...
                for (int dest_rank = 0; dest_rank < new_nprocs; dest_rank++) // ランク0以外の全てのランクに送る
                {
                    dest_rank_Px = dest_rank % Px;
                    dest_rank_Pz = dest_rank / Px;
                    for (int packing_loop = 0; packing_loop < lenz; packing_loop++)
                    {
                        size_t dest_addr = packing_loop * lenx + dest_rank * lenz * lenx + y * num * num;
                        size_t src_addr = y * num * num + dest_rank_Px * lenx + dest_rank_Pz * num * lenz + packing_loop * num;
                        memcpy(&packing_tmp_buffer[dest_addr], &y_axis_normal_plane_allprocs_H[src_addr], lenx * sizeof(float));
                    }
                }
            ...
            MPI_Iscatter(&packing_tmp_buffer[y * num * num], lenx * lenz, MPI_FLOAT, &y_axis_normal_plane_everyprocs_H[y * lenx * lenz], lenx * lenz, MPI_FLOAT, 0, CTCA_subcomm, &scatter_req[y]);
        ...
        MPI_Waitall(num, scatter_req, scatter_stat);
    ...
    if (new_rank == 0)
    {
        ...
        calc_measure_time(wrk_readarea_time_list, step_num, &wrk_readarea_ave_time, &wrk_readarea_max_gtime, &wrk_readarea_min_gtime);
