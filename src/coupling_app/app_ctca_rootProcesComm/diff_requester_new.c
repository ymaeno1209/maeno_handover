...
#define CON_PACKING_COUNT_MAX 32
...
int main(int argc, char *argv[])
{
    ...
    int loop_con_packing_max = num / con_packing_count;
    fprintf(stderr, "num=%d,con_packing_count=%d, loop_con_packing_max=%d\n", num, con_packing_count, loop_con_packing_max);
    cudaMalloc((void **)&f, sizeof(float) * ln);
    cudaMalloc((void **)&fn, sizeof(float) * ln);
    cudaMalloc((void **)&y_axis_normal_plane_perprocs, sizeof(float) * procs_surface_num * con_packing_count);
    ...
    for (int loop_con_packing = 0; loop_con_packing < loop_con_packing_max; loop_con_packing++)
    {
        ...
        if (new_rank == 0)
        {
            call_con_packing_new_rootrank(&data_allprocs_allstep[hdl_slot * whole_num + loop_con_packing * con_packing_count * nx * nz], &f[nx * nz + loop_con_packing * con_packing_count * nx * nz], num, con_packing_count, packing_stream);
        }
        else
        {
            call_con_packing_new(y_axis_normal_plane_perprocs, &f[nx * nz + loop_con_packing * con_packing_count * nx * nz], num, con_packing_count, packing_stream);
        }
        cudaStreamSynchronize(packing_stream);
        ...
        for (int i = 0; i < con_packing_count; i++)
        {
            if (new_rank == 0)
            {
                for (int src_rank = 0; src_rank < new_nprocs; src_rank++)
                {
                    MPI_Irecv();
                }
            }
