...
void gather_y_axis_normal_plane_D(float *y_axis_normal_plane_perprocs, float *y_axis_normal_plane_allprocs, int nx, int nz, int new_rank, int new_nprocs, MPI_Comm new_comm, MPI_Request *req, cudaStream_t packing_stream)
{
    ...
    if (new_rank == 0)
    {
        ...
        for (i = 1; i < new_nprocs; i++)
        {
            ...
            int irecv_tag = 0;
            MPI_Irecv(&y_axis_normal_plane_allprocs[nx * nz * (i - 1)], nx * nz, MPI_FLOAT, source_rank, irecv_tag, new_comm, &req[req_count++]);
            irecv_tag++;
        }
    }
    else
    {
        int dest_rank = 0;
        MPI_Isend(y_axis_normal_plane_perprocs, nx * nz, MPI_FLOAT, dest_rank, isend_tag, new_comm, &req[req_count++]);
        isend_tag++;
    }
}
