...
int main(int argc, char *argv[])
{
    ...
    int collect_data_count = (new_rank == 0) ? new_nprocs - 1 : 1;
    ...
    for (y = 0; y < ny; y++)
    {
        ...
        call_packing_data(y_axis_normal_plane_perprocs, f, nx, nx * y + nx * ny * nz, nx, -nx * ny, nz, packing_stream);
        ...
        // プロセス0にデータを集める
        gather_y_axis_normal_plane_D(y_axis_normal_plane_perprocs, &data_allprocs_allstep[hdl_slot * whole_num + y * whole_surface_num], nx, nz, new_rank, new_nprocs, CTCA_subcomm, collect_req, packing_stream);
        cudaStreamSynchronize(packing_stream);
        MPI_Waitall(collect_data_count, collect_req, collect_stat);
    ...
    if (new_rank == 0)
    {
        ...
        printf("Sim packing_global_time: ave=%f,max=%f,min=%f sec\n", req_packing_avg_gtime, req_packing_max_gtime, req_packing_min_gtime);
