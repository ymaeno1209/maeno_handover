...
int CTCAR_regarea_int(int *base, size_t size, int *areaid)
{
    ...
    return req_regarea((void *)base, size, size_byte, sizeof(int), GREA_INT, areaid);
}
...
int CTCAR_regarea_real4(float *base, size_t size, int *areaid)
{
    ...
    return req_regarea((void *)base, size, size_byte, sizeof(float), GREA_REAL4, areaid);
}
...
int CTCAR_regarea_real8(double *base, size_t size, int *areaid)
{
    ...
    return req_regarea((void *)base, size, size_byte, sizeof(double), DATA_REAL8, areaid);
}
...
// REQCPL_REQ_ENTRY(addr)  (*((int *)((char *)(addr) + REQCPL_REQ_OFFSET_ENTRY)))
// REQCPL_REQ_DATASIZE(addr)  (*((size_t *)((char *)(addr) + REQCPL_REQ_OFFSET_DATASIZE)))
// REQCPL_REQ_OFFSET_ENTRY=0
// REQCPL_REQ_OFFSET_DATASIZE=4
//reqentry = -1
//datasize = DAT_REAL8 = 2
    REQCPL_REQ_ENTRY(req_reqbuf) = reqentry;
    //reqentry[0]=-1
    REQCPL_REQ_DATASIZE(req_reqbuf) = datasize;
    //reqentry[1]=datasize
    //intparam{progid,i}={3,i}
    //REQCPL_REQ_SIZE=12
...
{
    //cpl_reqinfo_entry_mask=4095
    //*reqinfo = (*reqinfo & (1<<12)-1) | (1 << 12);
    //*reqinfo & 1<<12-1は下位12ビットだけを保持し、13ビット目以降は0にする
    //
...
{
    //cpl_reqinfo_entry_mask=4095
    //*reqinfo & cpl_reqinfo_entry_mask = {reqentry,intparamnum,datasize}
...
{
    //*reqinfo = (*reqinfo & ~((1<<12) -1)) | (cpl_reqbuf & (1<12) -1)
    //~は1と0の反転を意味しているので、~((1<<12) -1)は下位12ビットが0、13ビット目以降が1
            ...
            if (flag) {
                ...
                case TAG_REQ:
                    ...
                    *fromrank = subrank_table[stat.MPI_SOURCE];
                    ...
                    //reqinfo={reqentry,intparamnum,datasize,stat,MPI_SOURCE}
                    //intparam={progid,i}
                    if (reqdatasize > 0) 
...
int cpl_enqreq(int *reqinfo, int progid, int *intparam, int intparamnum, void *data, size_t datasz)
{
    ...
    //reqitem={stat.MPI_SOURRCE,progid,reqentry,intparamnum,datasize,progid,i}
...
int CTCAC_readarea_int(int areaid, int reqrank, size_t offset, size_t size, int *dest)
{
    ...
    if (readarea(areaid, reqrank, offset, size, dest, GREA_INT) < 0) {
...
int CTCAC_readarea_real4(int areaid, int reqrank, size_t offset, size_t size, float *dest)
{
    ...
    if (readarea(areaid, reqrank, offset, size, dest, GREA_REAL4) < 0) {
...
int CTCAC_readarea_real8(int areaid, int reqrank, size_t offset, size_t size, double *dest)
{
    ...
    if (readarea(areaid, reqrank, offset, size, dest, DATA_REAL8) < 0) {
...
int CTCAC_writearea_int(int areaid, int reqrank, size_t offset, size_t size, int *src)
{
    ...
    if (writearea(areaid, reqrank, offset, size, src, GREA_INT) < 0) {
...
int CTCAC_writearea_real4(int areaid, int reqrank, size_t offset, size_t size, float *src)
{
    ...
    if (writearea(areaid, reqrank, offset, size, src, GREA_REAL4) < 0) {
...
int CTCAC_writearea_real8(int areaid, int reqrank, size_t offset, size_t size, double *src)
{
    ...
    if (writearea(areaid, reqrank, offset, size, src, DATA_REAL8) < 0) {
