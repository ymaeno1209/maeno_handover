...
static int *rank_wrkcomm_table;
static int *req_wgid_table;
static int *cpl_wgid_table;
static MPI_Win win_wgid;
static int cpl_reqnprocs;
static int max_req_maxreqs;
static int numwrkcomms;
...
void garea_attach(int gareaid, void *base, size_t size_byte);
...
void setup_garea_common_tables(int num_gareas);
void initialize_garea_base_addresses(int num_gareas);
void initialize_garea(int gareaidctr);
void free_garea_common_tables();
void validate_gareaid(int gareaid, char *function_name);
...
int get_grank_req(int localrank, int *grank)
{
    int i;

    for (i = 0; i < world_nprocs; i++)
    {
        if (rank_wrkcomm_table[i] == numwrkcomms + 1)
            if (subrank_table[i] == localrank)
            {
                *grank = i;
                return 0;
            }
    }
    fprintf(stderr, "%d: get_grank_req: couldn't find localrank %d\n", world_myrank, localrank);
}

int get_grank_cpl(int *grank)
{
    *grank = rank_cpl;
...
int get_grank_wrk(int wgid, int localrank, int *grank)
{
    int i;
    for (i = 0; i < world_nprocs; i++)
    {
        if (rank_wrkcomm_table[i] == wgid)
            if (subrank_table[i] == localrank)
            {
                *grank = i;
            }
...
int CTCAR_init_detail(int numareas, int numreqs, int intparams)
{
    ...
    //  split communicator
    MPI_Bcast(&numwrkcomms, 1, MPI_INT, rank_cpl, MPI_COMM_WORLD);
    ...
    // Calculate and share max of req_maxreqs
    MPI_Allreduce(&req_maxreqs, &max_req_maxreqs, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);

    // setup workgroup ID table
    size_byte = req_maxreqs * sizeof(int);
    MPI_Alloc_mem(size_byte, MPI_INFO_NULL, &req_wgid_table);
    for (i = 0; i < req_maxreqs; i++)
        req_wgid_table[i] = WGID_NULL;
    MPI_Win_create((void *)req_wgid_table, size_byte, sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win_wgid);
    MPI_Win_lock_all(MPI_MODE_NOCHECK, win_wgid);
...
int CTCAR_finalize()
{
    ...
    MPI_Win_unlock_all(win_wgid);
    MPI_Win_free(&win_reqstat);
    MPI_Win_free(&win_wgid);
...
int CTCAR_get_grank_req(int localrank, int *grank)
{
    return get_grank_req(localrank, grank);
}

int CTCAR_get_grank_cpl(int *grank)
{
    return get_grank_cpl(grank);
}

int CTCAR_get_grank_wrk(int wgid, int localrank, int *grank)
{
    return get_grank_wrk(wgid, localrank, grank);
}

int CTCAR_get_wgid(int64_t handle, int *wgid)
{
    int i;
    for (i = 0; i < req_maxreqs; i++)
        if (req_reqstat_table[i] == handle)
        {
            *wgid = req_wgid_table[i];
            return 0;
        }
    fprintf(stderr, "Error: no such handle %ld\n", handle);
    return -1;
}
...
int CTCAC_init_detail(int numareas, int numreqs, int intparams, size_t bufslotsz, int bufslotnum)
{
    ...
    int *progid_numprocs_table, *progid_procspercomm_table;
    ...
    // Calculate and share max of req_maxreqs
    val = 0;
    MPI_Allreduce(&val, &max_req_maxreqs, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
    ...
    for (i = 0; i < world_nprocs; i++)
        if (role_table[i] == ROLE_REQ)
            cpl_reqnprocs++;
    size_byte = cpl_reqnprocs * max_req_maxreqs * sizeof(int);
    MPI_Alloc_mem(size_byte, MPI_INFO_NULL, &cpl_wgid_table);
    for (i = 0; i < cpl_reqnprocs * max_req_maxreqs; i++)
        cpl_wgid_table[i] = WGID_NULL;
    MPI_Win_create((void *)cpl_wgid_table, size_byte, sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win_wgid);
    MPI_Win_lock_all(MPI_MODE_NOCHECK, win_wgid);
...
int readarea(int areaid, int reqrank, size_t offset, size_t size, void *dest, int type)
{
    ...
    while (size_remain > 0)
    {
        ...
        // printf("addr=%p,size_toget=%lu,targetrank=%d,disp=%lu,my_world_rank=%d\n", addr, size_toget, targetrank, disp, world_myrank);
    ...
    // end_time = MPI_Wtime();
    // printf("Read area time: %f (sec), cnt=%d\n", end_time - start_time, cnt);
...
int CTCAC_get_grank_req(int localrank, int *grank)
{
    return get_grank_req(localrank, grank);
}

int CTCAC_get_grank_cpl(int *grank)
{
    return get_grank_cpl(grank);
}

int CTCAC_get_grank_wrk(int wgid, int localrank, int *grank)
{
    return get_grank_wrk(wgid, localrank, grank);
}

int CTCAC_get_wgid(int reqinfo, int *wgid)
{
    int fromrank, reqentry;
    fromrank = CTCAC_reqinfo_get_fromrank(&reqinfo);
    reqentry = CTCAC_reqinfo_get_entry(&reqinfo);
    *wgid = cpl_wgid_table[fromrank * max_req_maxreqs + reqentry];
    return 0;
}

// CTCAW_init_detail(progid, procspercomm, DEF_MAXNUMAREAS, DEF_MAXINTPARAMS);
// numareas = DEF_MAXNUMAREAS = 10
// intparams = DEF_MAXINTPARAMS = 10
int CTCAW_init_detail(int progid, int procspercomm, int numareas, int intparams)
{
    int i, sub_myrank, val;
    int *rank_progid_table, *rank_procspercomm_table;
    MPI_Aint size_byte;
    ...
    // setup workgroup ID table
    size_byte = 0;
    MPI_Win_create(NULL, size_byte, sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win_wgid);
    MPI_Win_lock_all(MPI_MODE_NOCHECK, win_wgid);
...
int wrk_pollreq(int *fromrank, int *intparam, int intparamnum, void *data, size_t datasz)
{
    ...
    // write my workgroup ID to wgid tables
    if (submyrank == 0 && wrk_entry >= 0)
    {
        // write to requester
        disp = wrk_entry;
        MPI_Put(&wrk_myworkcomm, 1, MPI_INT, wrk_fromrank, disp, 1, MPI_INT, win_wgid);
        MPI_Win_flush(wrk_fromrank, win_wgid);
        // write to coupler
        disp = wrk_fromrank * max_req_maxreqs + wrk_entry;
        MPI_Put(&wrk_myworkcomm, 1, MPI_INT, rank_cpl, disp, 1, MPI_INT, win_wgid);
        MPI_Win_flush(rank_cpl, win_wgid);
    }
...
int CTCAW_complete()
{
    ...
    if (submyrank == 0)
    {
        ...
        if (wrk_entry >= 0)
        {
            ...
            // write WGID_NULL ID to wgid tables
            val = WGID_NULL;
            disp = wrk_entry;
            MPI_Put(&val, 1, MPI_INT, wrk_fromrank, disp, 1, MPI_INT, win_wgid);
            MPI_Win_flush(wrk_fromrank, win_wgid);
            disp = wrk_fromrank * max_req_maxreqs + wrk_entry;
            MPI_Put(&val, 1, MPI_INT, rank_cpl, disp, 1, MPI_INT, win_wgid);
            MPI_Win_flush(rank_cpl, win_wgid);
        }
    }
...
int CTCAW_get_grank_req(int localrank, int *grank)
{
    return get_grank_req(localrank, grank);
}

int CTCAW_get_grank_cpl(int *grank)
{
    return get_grank_cpl(grank);
}

int CTCAW_get_grank_wrk(int wgid, int localrank, int *grank)
{
    return get_grank_wrk(wgid, localrank, grank);
}
...
void CTCAR_garea_attach(int gareaid, void *base, size_t size_byte)
{
    garea_attach(gareaid, base, size_byte);
}

void CTCAC_garea_attach(int gareaid, void *base, size_t size_byte)
{
    garea_attach(gareaid, base, size_byte);
}

void CTCAW_garea_attach(int gareaid, void *base, size_t size_byte)
{
    garea_attach(gareaid, base, size_byte);
}
...
void garea_create(int num_gareas, int *gareaid)
{
    ...
    *gareaid = gareaidctr;
    ...
    if (gareaidctr == 0)
    {
        setup_garea_common_tables(num_gareas);
    }
...
void garea_attach(int gareaid, void *local_base, size_t size_byte)
{
    MPI_Aint base_toshare;
    int target_rank, target_disp;

    validate_gareaid(gareaid, "garea_attach");
    MPI_Get_address(local_base, &base_toshare);

    MPI_Win_attach(win_garea_table[gareaid], local_base, size_byte);

    target_disp = gareaid * world_nprocs + world_myrank;
    for (target_rank = 0; target_rank < world_nprocs; target_rank++)
    {
        MPI_Put(&base_toshare, GAREA_BASE_ADDRESSES_ELEMENT_PER_WIN, MPI_AINT, target_rank, target_disp, GAREA_BASE_ADDRESSES_ELEMENT_PER_WIN, MPI_AINT, win_garea_base_addresses);
    }
...
void garea_detach(int gareaid, void *base)
{
    MPI_Aint base_toshare;
    int target_rank, target_disp;
    int base_init = GAREA_BASE_ADDRESSES_INITIAL;
    validate_gareaid(gareaid, "garea_detach");

    MPI_Win_detach(win_garea_table[gareaid], base);
    target_disp = gareaid * world_nprocs + world_myrank;
    for (target_rank = 0; target_rank < world_nprocs; target_rank++)
    {
        MPI_Put(&base_init, GAREA_BASE_ADDRESSES_ELEMENT_PER_WIN, MPI_AINT, target_rank, target_disp, GAREA_BASE_ADDRESSES_ELEMENT_PER_WIN, MPI_AINT, win_garea_base_addresses);
    }
    MPI_Win_flush_all(win_garea_table[gareaid]);
    MPI_Win_flush_all(win_garea_base_addresses);
}
...
{
    size_t data_element_remain, element_toget, disp, target_base_address;
    MPI_Datatype mpitype;
    char *addr;
    int unitsize;

    validate_gareaid(gareaid, "garea_write");
    ...
    addr = src_addr;
    disp = target_base_address + offset * (size_t)unitsize;

    while (data_element_remain > 0)
    {
        if ((data_element_remain * (size_t)unitsize) > (size_t)MAX_TRANSFER_SIZE)
        {
            element_toget = (size_t)MAX_TRANSFER_SIZE / (size_t)unitsize;
        }
        else
        {
        ...
        addr += element_toget * (size_t)unitsize;
        disp += element_toget;
    }
...
void garea_read(int gareaid, int target_world_rank, size_t offset, size_t data_element, void *dest_addr, int type)
{
    ...
    validate_gareaid(gareaid, "garea_read");
...
{
    int i;
    garea_base_addresses_byte = world_nprocs * num_gareas * sizeof(MPI_Aint);
    garea_base_addresses = (MPI_Aint *)malloc(garea_base_addresses_byte);
    for (i = 0; i < num_gareas * world_nprocs; i++)
        garea_base_addresses[i] = (MPI_Aint)GAREA_BASE_ADDRESSES_INITIAL;
    MPI_Win_create((void *)garea_base_addresses, garea_base_addresses_byte, sizeof(MPI_Aint), MPI_INFO_NULL, MPI_COMM_WORLD, &win_garea_base_addresses);
...
MPI_Aint fetch_remote_base_address(int gareaid, int target_world_rank)
{
    ...
    int offset_tofetch_remote_base_address = gareaid * world_nprocs + target_world_rank;

    remote_base_address = garea_base_addresses[offset_tofetch_remote_base_address];
...
void validate_gareaid(int gareaid, char *function_name)
{
    ...
    if (gareaid > DEF_MAXNUM_GAREAS - 1)
    {
        fprintf(stderr, "%d : %s : ERROR : wrong gareaid\n", world_myrank);
        fprintf(stderr, "gareaidは%dより小さい値を入力してください\ns", DEF_MAXNUM_GAREAS);
        MPI_Abort(MPI_COMM_WORLD, 0);
...
void show_base_addresses()
{
    printf("world_myrank %d: base_address\n", world_myrank);
    for (int i = 0; i < DEF_MAXNUM_GAREAS; i++)
    {
        printf("garea[%d]: ", i);
        for (int j = 0; j < world_nprocs; j++)
        {
            printf("[%d]=%lx", i * world_nprocs + j, (long)garea_base_addresses[i * world_nprocs + j]);
            if (j != world_nprocs - 1)
                printf(",");
        }
        printf("\n");
